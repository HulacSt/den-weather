...
high_percentile_string <- str_glue("Today's high of __{dy$actual_high}F__ was in the __{nom_ord(round(high_percentile*100))} percentile__ of highs in similar days.")
...
comparable |> 
  ...
  ggtitle('Today\'s high vs highs of comparable days')
...
high_last_time_string <- str_glue("The last time the high was as high (or higher) as today was __{last_time_this_warm_or_warmer$date}__, when the high was __{last_time_this_warm_or_warmer$actual_high}F__.")
```
...
low_percentile_string <- str_glue("Today's low of __{dy$actual_low}F__ was in the __{nom_ord(round(low_percentile*100))} percentile__ of lows in similar days.")
...
comparable |> 
  ...
  ggtitle('Today\'s low vs lows of comparable days')
...
low_last_time_string <- str_glue("The last time the low was as low (or lower) as today was __{last_time_this_cold_or_colder$date}__, when the high was __{last_time_this_cold_or_colder$actual_low}F__.")
```
